...
docker-build: ## Build Docker image
	...
	@docker build -t $(DOCKER_REGISTRY)/$(tenant-service):$(VERSION) .
	@docker tag $(DOCKER_REGISTRY)/$(tenant-service):$(VERSION) $(DOCKER_REGISTRY)/$(tenant-service):latest
	@echo "Docker image built: $(DOCKER_REGISTRY)/$(tenant-service):$(VERSION)"
...
docker-run: ## Run Docker container locally
	...
	@docker run --rm -p 8080:8080 -p 50051:50051 \
		--name $(tenant-service) \
		$(DOCKER_REGISTRY)/$(tenant-service):latest
